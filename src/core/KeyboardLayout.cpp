#include "KeyboardLayout.h"
#include "TextUtils.h"

#include <initializer_list>

std::vector<KeyLayout> BuildKeyboardLayout(Language language) {
    std::vector<KeyLayout> keys;

    auto addKey = [&keys](int row, int key, const std::string& label, float width = 1.0f) {
        const FingerType finger = GetFingerForKey(key);
        keys.push_back({
            key,
            label,
            row,
            width,
            finger
        });
    };

    auto addUtf8Row = [&addKey](int row, const std::string& rowText) {
        for (int key : Utf8ToCodepoints(rowText)) {
            addKey(row, key, CodepointToUtf8(key));
        }
    };

    auto addCodepointRow = [&addKey](int row, std::initializer_list<int> rowKeys) {
        for (int key : rowKeys) {
            addKey(row, key, CodepointToUtf8(key));
        }
    };

    addUtf8Row(0, R"keys(!?:;'"<>()[]{}\|/)keys");

    if (language == Language::Russian) {
        addCodepointRow(1, { 0x0439, 0x0446, 0x0443, 0x043a, 0x0435, 0x043d, 0x0433, 0x0448, 0x0449, 0x0437, 0x0445, 0x044a });
        addCodepointRow(2, { 0x0444, 0x044b, 0x0432, 0x0430, 0x043f, 0x0440, 0x043e, 0x043b, 0x0434, 0x0436, 0x044d });
        addCodepointRow(3, { 0x044f, 0x0447, 0x0441, 0x043c, 0x0438, 0x0442, 0x044c, 0x0431, 0x044e, ',', '.', '-' });
    } else {
        addUtf8Row(1, "qwertyuiop");
        addUtf8Row(2, "asdfghjkl");
        addUtf8Row(3, "zxcvbnm,.-");
    }

    addKey(4, ' ', language == Language::Russian ? u8"ПРОБЕЛ" : "SPACE", 5.2f);
    addKey(4, '\n', "ENTER", 2.0f);

    return keys;
}

FingerType GetFingerForKey(int key) {
    switch (ToLowerCodepoint(key)) {
        case 'q':
        case '!':
        case 0x0439:
        case 'a':
        case 0x0444:
        case 'z':
        case 0x044f:
            return FingerType::LeftPinky;
        case 'w':
        case 0x0446:
        case 's':
        case 0x044b:
        case 'x':
        case 0x0447:
            return FingerType::LeftRing;
        case 'e':
        case 0x0443:
        case 'd':
        case 0x0432:
        case 'c':
        case 0x0441:
            return FingerType::LeftMiddle;
        case ',':
            return FingerType::RightMiddle;
        case 'r':
        case '<':
        case '[':
        case '{':
        case 0x043a:
        case 't':
        case 0x0435:
        case 'f':
        case 0x0430:
        case 'g':
        case 0x043f:
        case 'v':
        case 0x043c:
        case 'b':
        case 0x0438:
            return FingerType::LeftIndex;
        case ' ':
            return FingerType::RightThumb;
        case 'y':
        case '>':
        case '(':
        case ')':
        case ']':
        case '}':
        case 0x043d:
        case 'u':
        case 0x0433:
        case 'h':
        case 0x0440:
        case 'j':
        case 0x043e:
        case 'n':
        case 0x0442:
        case 'm':
        case 0x044c:
            return FingerType::RightIndex;
        case 'i':
        case 0x0448:
        case 'k':
        case 0x043b:
            return FingerType::RightMiddle;
        case 'o':
        case ':':
        case '?':
        case 0x0449:
        case 'l':
        case 0x0434:
        case '.':
            return FingerType::RightRing;
        case 'p':
        case '\n':
        case '\'':
        case '"':
        case '\\':
        case '|':
        case '/':
        case 0x0437:
        case 0x0445:
        case 0x044a:
        case ';':
        case '-':
        case 0x0436:
        case 0x044d:
        case 0x0431:
        case 0x044e:
            return FingerType::RightPinky;
        default:
            return FingerType::None;
    }
}

Color GetFingerColor(FingerType finger, const Theme& theme) {
    switch (finger) {
        case FingerType::LeftPinky:
        case FingerType::RightPinky:
            return theme.Fingers.Pinky;
        case FingerType::LeftRing:
        case FingerType::RightRing:
            return theme.Fingers.Ring;
        case FingerType::LeftMiddle:
        case FingerType::RightMiddle:
            return theme.Fingers.Middle;
        case FingerType::LeftIndex:
        case FingerType::RightIndex:
            return theme.Fingers.Index;
        case FingerType::LeftThumb:
        case FingerType::RightThumb:
            return theme.Fingers.Thumb;
        default:
            return theme.TextDefault;
    }
}
